...
Ruff and mypy: 1422 passed / 1 skipped locally (optional PyArrow present); under
...
the engine loads all sixteen migrations (`a001`..`d016`) from an installed wheel.
...
`sports-quant` console script, and `db-init` (twice, schema v16) all resolve from
...
stays v16 (migration `d016_kalshi_matching`); **Phase E1 (point-in-time foundation) and Phase E2 (dataset + quality commands) are both complete and independently reviewed; Phase E is complete. F0 (Phase F research planning) is complete — see `PHASE_F_RESEARCH_PLAN.md` and `PHASE_F_FEATURE_CONTRACT.md`; no Phase F implementation, corpus backfill, feature engineering, model training, calibration, simulation, or recommendation work has started, schema remains v16, and no live provider request or persisted ingestion occurred during F0. F0 has since been independently reviewed (`PHASE_F_RESEARCH_PLAN.md` §R): ordinary retrospective backfill cannot yield strict point-in-time feature rows (observations receive today's `observed_at`), so the corpus strategy is a staged hybrid (forward strict-PIT + a separate, explicitly non-PIT reconstructed-research corpus), and the live pilot is split into F1A (request/credit controls, not yet built) then F1B and is NOT yet authorized; schema remains v16.**
...
D5B2 Kalshi matching, each independently reviewed); Phase E1 (point-in-time foundation) and Phase E2 (dataset + quality commands) are both complete and independently reviewed; Phase E is complete. F0 (Phase F research planning) is complete — see `PHASE_F_RESEARCH_PLAN.md` and `PHASE_F_FEATURE_CONTRACT.md`; no Phase F implementation, corpus backfill, feature engineering, model training, calibration, simulation, or recommendation work has started, schema remains v16, and no live provider request or persisted ingestion occurred during F0. F0 has since been independently reviewed (`PHASE_F_RESEARCH_PLAN.md` §R): ordinary retrospective backfill cannot yield strict point-in-time feature rows (observations receive today's `observed_at`), so the corpus strategy is a staged hybrid (forward strict-PIT + a separate, explicitly non-PIT reconstructed-research corpus), and the live pilot is split into F1A (request/credit controls, not yet built) then F1B and is NOT yet authorized; schema remains v16.**
...
ingestion or weather backfill has occurred. D5A canonical team/player/venue/official-game matching is complete (deterministic alias + official-schedule evidence, mocked/offline); D5B1 sportsbook-event matching is complete (migration `d015_sportsbook_matching`, mocked/offline); D5B2 Kalshi event + game-winner market matching is complete (schema v16, migration `d016_kalshi_matching`; mocked/offline plus a bounded public-contract audit and parser smoke); Phase E1 (point-in-time foundation) and Phase E2 (dataset + quality commands) are both complete and independently reviewed; Phase E is complete. F0 (Phase F research planning) is complete — see `PHASE_F_RESEARCH_PLAN.md` and `PHASE_F_FEATURE_CONTRACT.md`; no Phase F implementation, corpus backfill, feature engineering, model training, calibration, simulation, or recommendation work has started, schema remains v16, and no live provider request or persisted ingestion occurred during F0. F0 has since been independently reviewed (`PHASE_F_RESEARCH_PLAN.md` §R): ordinary retrospective backfill cannot yield strict point-in-time feature rows (observations receive today's `observed_at`), so the corpus strategy is a staged hybrid (forward strict-PIT + a separate, explicitly non-PIT reconstructed-research corpus), and the live pilot is split into F1A (request/credit controls, not yet built) then F1B and is NOT yet authorized; schema remains v16.** D2
...
performed; D5A canonical team/player/venue/official-game matching is complete (deterministic alias + official-schedule evidence, mocked/offline); D5B1 sportsbook-event matching is complete (migration `d015_sportsbook_matching`, mocked/offline); D5B2 Kalshi event + game-winner market matching is complete (schema v16, migration `d016_kalshi_matching`; mocked/offline plus a bounded public-contract audit and parser smoke); Phase E1 (point-in-time foundation) is complete and Phase E2 (dataset + quality commands) is complete and independently reviewed (Phase E complete; no later phase started). The live MLB provider audit and
...
no persisted weather ingestion or backfill). D5A canonical team/player/venue/official-game matching is complete (deterministic alias + official-schedule evidence, mocked/offline); D5B1 sportsbook-event matching is complete (migration `d015_sportsbook_matching`, mocked/offline); D5B2 Kalshi event + game-winner market matching is complete (schema v16, migration `d016_kalshi_matching`; mocked/offline plus a bounded public-contract audit and parser smoke); Phase E1 (point-in-time foundation) is complete and Phase E2 (dataset + quality commands) is complete and independently reviewed (Phase E complete; no later phase started).
...
| E | Point-in-time builder, quality rules, leakage tests | ✅ **E complete — independently reviewed** — E1: point-in-time as-of accessors, fail-closed safe-join registry (with feature-column allowlists + positive-allowlist SQL grammar), evaluation-only closing-line isolation, content-hash fail-closed ties, review-gated identity, true read-only URI mode, and adversarial DQ-PIT-001..011 guards (`sports_quant/pit/`, schema v16, no new migration). **E2 complete — independently reviewed**: `pit/dataset.py` historical row builder (visible-at-cutoff pregame cutoff, strictly-after-cutoff label, structural feature-state/label split, zero-column `GameStateDataset` conversion with NaN `true_prob`, microsecond timestamps), `sports_quant/quality/` rules+report with registry-derived equal-time conflict scan, and offline read-only `data-status`/`data-quality` CLI commands that fail closed on an uncheckpointed WAL and gate validity/exit on open blocking issues (schema v16, no new migration). No later phase started. |
...
**D5B2** Kalshi event/market matching is now implemented (schema v16,
`d016_kalshi_matching`; see the D5B2 section below). No live provider request,
persisted provider ingestion, or historical backfill was performed; Phase E has
not started.

**D5B1 — deterministic sportsbook-event matching (complete, mocked/offline).**
Already-ingested The Odds API `sportsbook_events` resolve to the existing
canonical `games` using provider-scoped (`the_odds_api`) team aliases and
venue-aware schedule/time evidence — never a price, implied probability,
bookmaker count, final score, or settled outcome (the matcher does not import or
read `sportsbook_price_snapshots`). Tiers: `schedule_key_exact` 0.95 (±90 min),
`schedule_key_window` 0.88 (±12 h), `schedule_key_swapped` 0.85 (neutral-site
only, manual-review gated, `DQ-MATCH-007`); a non-neutral reversed orientation is
blocking (`DQ-MATCH-003`); the provider event id is never an official key.
Migration **d015** (schema v15) adds `sportsbook_events.match_decision_id` (the
exact accepted decision) and a typed `orientation` (`direct`/`swapped`), paired
with `game_id` and non-regressing. Direct orientation is orientation-approved for
canonical use; a neutral swapped match is not until reviewed. Existing h2h /
spreads / totals outcomes are validated against the accepted orientation and
unknown/malformed ones surfaced via `DQ-SB-OUTCOME-001` (never dropped or
...
**D5B2 — deterministic Kalshi event + game-winner market matching (complete,
...
`yes_team_id` + `matched_rules_hash` + typed `market_semantic`. Only game-winner
markets are linked automatically; spreads/totals/props/period/combo markets are
retained and reported as *unsupported semantics*, never mislabeled. A rules
change invalidates readiness via a blocking `DQ-MATCH-004`;
`is_kalshi_market_orientation_approved()` is fail-closed and as-of correct.
`match-markets` is local/network-free and dry-run persists nothing. **No
authenticated Kalshi access, account/order surface, live request, or Phase E work
was performed. Phase E1 (point-in-time foundation) and Phase E2 (dataset + quality commands) are both complete and independently reviewed; Phase E is complete. F0 (Phase F research planning) is complete — see `PHASE_F_RESEARCH_PLAN.md` and `PHASE_F_FEATURE_CONTRACT.md`; no Phase F implementation, corpus backfill, feature engineering, model training, calibration, simulation, or recommendation work has started, schema remains v16, and no live provider request or persisted ingestion occurred during F0. F0 has since been independently reviewed (`PHASE_F_RESEARCH_PLAN.md` §R): ordinary retrospective backfill cannot yield strict point-in-time feature rows (observations receive today's `observed_at`), so the corpus strategy is a staged hybrid (forward strict-PIT + a separate, explicitly non-PIT reconstructed-research corpus), and the live pilot is split into F1A (request/credit controls, not yet built) then F1B and is NOT yet authorized; schema remains v16.**
...
> verified; no persisted weather ingestion/backfill); D5A canonical team/player/venue/official-game matching is complete (deterministic alias + official-schedule evidence, mocked/offline); D5B1 sportsbook-event matching is complete (migration `d015_sportsbook_matching`, mocked/offline); D5B2 Kalshi event + game-winner market matching is complete (schema v16, migration `d016_kalshi_matching`; mocked/offline plus a bounded public-contract audit and parser smoke); Phase E1 (point-in-time foundation) is complete and Phase E2 (dataset + quality commands) is complete and independently reviewed (Phase E complete; no later phase started). The
...
tables), `d012_nba_specifics` (v12 — D3's `nba_quarter_lines` / `injury_snapshots`
/ `play_snapshots`), and `d013_nba_typed_repairs` (v13 — the D3 correctness
...
**Create (as implemented):** `sports_quant/pit/{__init__,asof,dataset,evaluation_only,models,registry}.py`;
`sports_quant/quality/{__init__,rules,report,runner}.py`; `sports_quant/status.py`;
`sports_quant/report_access.py`;
`sports_quant/pit/tests/{test_asof,test_dataset,test_leakage,test_determinism,test_repair,test_audit}.py`;
`sports_quant/quality/tests/{test_rules,test_cli,test_audit_e2}.py`.
**No `e001_data_quality.sql` migration was created** — the `data_quality_issues`
table and its repository already exist from Phase C/D (schema v16); the legacy plan
line predates that. E2 adds no migration; schema stays v16.
...
| Two observations share `observed_at` | The `games` current-state denormalization still tie-breaks with `status_id`, but that column is **forbidden for point-in-time feature reads**. The E1/E2 point-in-time layer resolves an equal-`observed_at` tie by **`content_hash`**: identical content returns one row; DIFFERENT content is a genuine conflict that **fails closed** (`AsOfAmbiguityError`) — never a ULID/"most-recently-recorded" winner (a generated id is not rebuild-stable across a fresh corpus). |
| `original_start` | Never changed — now a database rule, not a convention |

The tie-break matters more than it looks. A generated ULID is stable inside ONE
database but differs across a fresh rebuild, so it is **not** a valid semantic
winner for conflicting equal-time content. The point-in-time layer therefore
compares `content_hash`: matching content is deduplicated to one deterministic
row and conflicting content fails closed, so a rebuilt corpus never silently
disagrees.

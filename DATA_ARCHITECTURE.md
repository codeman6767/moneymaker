...
| 018 | `f018_retrospective_provenance` *(Phase F — built)* | adds the five append-only Lane-R (retrospective research) provenance tables: `reconstruction_corpus_versions` (one reproducible reconstruction context, identified by a semantic digest over source fingerprint / target set / policies / G1 variant; supersession APPENDS and never rewrites), `identity_audit_records` (the stored result of one corpus-scoped G5 identity audit, where an `accepted` verdict is refused by CHECK unless collisions are 0 AND the provider API generation is verified), `identity_audit_findings` (per-finding severity, classification and observed blast radius; sanitized detail only, never a provider body), `static_crosswalk_provenance` (a provider key bound to a canonical entity by a cleared audit — no name, affiliation, position, roster or outcome column exists, and triggers enforce entity-type AND league correctness), and `reconstructed_input_provenance` (per-target certification of an input family against `provenance_class` + `availability_basis`, storing NO feature value, NO `availability_confidence` and NO materialized `effective_at`). UPDATE/DELETE refused by trigger on all five. Strict PIT is unaffected: all five are `unsupported` joins in the safe-join registry and `AsOfReader` is untouched — schema v18 |
| 019 | `f019_retrospective_provenance_repairs` *(Phase F review — built)* | trigger-only repairs proven by the independent review of f018 (`RETROSPECTIVE_PIT_SCHEMA_V18_INDEPENDENT_REVIEW.md`). f018 is preserved byte-for-byte: applied migrations are immutable, and SQLite cannot ADD CONSTRAINT, so every repair is a BEFORE INSERT trigger enforcing the same predicate against any writer. Closes: a static crosswalk citing an accepted identity audit taken over a DIFFERENT source corpus (a one-month audit vouching for five seasons); an ACCEPTED audit acquiring a contradictory blocking identity_collision finding; an ELIGIBLE reconstructed input carrying no source-evidence pointer (EXCLUDED rows stay exempt); `source_evidence_table` accepting any string (now a fixed allowlist of append-only observation tables); shape-only timestamps that admitted month 99, Feb 30, lower-case `z` and offsets into lexicographically-ordered TEXT columns; and cross-league supersession enforced only in the repository — schema v19 |
...
reconciled to the Phase B/C provenance conventions (universal
`observed_at`/`ingested_at`/`raw_response` provenance; append-only snapshots;
c008 first/current provenance for mutable current-state; `match_candidates` as a
normalized child table rather than a `candidates_json` blob).
...
✅    d011_official_games_stats.sql                                 (Phase D2, built)
◻     d012_nba_specifics.sql  d013_weather.sql                      (Phase D, planned)
    repositories/
✅    __init__.py  base.py
...
◻     references.py  venues.py  matching.py  data_quality.py   (Phase D, planned)
    seeds/

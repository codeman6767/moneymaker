...
> audit and parser smoke). Phase D is complete; Phase E1 (point-in-time foundation) and Phase E2 (dataset + quality commands) are both complete and independently reviewed; Phase E is complete. F0 (Phase F research planning) is complete — see `PHASE_F_RESEARCH_PLAN.md`; no Phase F implementation, corpus backfill, feature engineering, model training, simulation, or recommendation work has started, schema remains v16, and no live provider request or persisted ingestion occurred during F0. F0 has since been independently reviewed (`PHASE_F_RESEARCH_PLAN.md` §R): ordinary retrospective backfill cannot yield strict point-in-time feature rows, so the corpus strategy is a staged hybrid (forward strict-PIT + a separate non-PIT reconstructed-research corpus), and the live pilot is split into F1A (request/credit controls, not yet built) then F1B and is NOT yet authorized; schema remains v16.** D1
...
> Kalshi contract audit and parser smoke); Phase E1 (point-in-time foundation) and Phase E2 (dataset + quality commands) are both complete and independently reviewed; Phase E is complete. F0 (Phase F research planning) is complete — see `PHASE_F_RESEARCH_PLAN.md`; no Phase F implementation, corpus backfill, feature engineering, model training, simulation, or recommendation work has started, schema remains v16, and no live provider request or persisted ingestion occurred during F0. F0 has since been independently reviewed (`PHASE_F_RESEARCH_PLAN.md` §R): ordinary retrospective backfill cannot yield strict point-in-time feature rows, so the corpus strategy is a staged hybrid (forward strict-PIT + a separate non-PIT reconstructed-research corpus), and the live pilot is split into F1A (request/credit controls, not yet built) then F1B and is NOT yet authorized; schema remains v16.** D5A shipped as the
...
> reviewed at schema version 16; Phase E1 (point-in-time foundation) and Phase E2 (dataset + quality commands) are both complete and independently reviewed; Phase E is complete. F0 (Phase F research planning) is complete — see `PHASE_F_RESEARCH_PLAN.md`; no Phase F implementation, corpus backfill, feature engineering, model training, simulation, or recommendation work has started, schema remains v16, and no live provider request or persisted ingestion occurred during F0. F0 has since been independently reviewed (`PHASE_F_RESEARCH_PLAN.md` §R): ordinary retrospective backfill cannot yield strict point-in-time feature rows, so the corpus strategy is a staged hybrid (forward strict-PIT + a separate non-PIT reconstructed-research corpus), and the live pilot is split into F1A (request/credit controls, not yet built) then F1B and is NOT yet authorized; schema remains v16.**
>
...
> adds the `sports_quant/pit/` + `sports_quant/quality/` suites; the current full-suite total is 1422 passed
> / 1 skipped.)
>
